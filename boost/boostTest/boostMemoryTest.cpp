...
0x5576b87d2d40 -> load p_main_2 b87d2cf0

start load
 call person(int) construct 20
0x5576b87d2b30 -> load p_save 20	// 3. 
0x5576b87d2b30 -> load p_save 0	// load 中 打印 delete 后的内存变量，为0，说明在 boost 函数内部调用的 delete 会自动清理内存
 call person(int) construct 1f
0x5576b87d2b30 -> load p before read 
call person() construct
0x5576b87d30b0 -> load p after read 
1f
 call person(int) construct 1f
...
/*
结论：
1. boost 在 load 对象指针时，会新分配内存，而不是使用手动 new 出来的内存，并且指向 new 内存的指针会丢失，导致内存泄漏
