...
/**
 * @brief 利用观测到该特征点的所有位姿来三角化特征点，（计算的是特征点在上一帧坐标系下的深度）
 *
 * @param[in] Ps
 * @param[in] tic
 * @param[in] ric
...
void FeatureManager::triangulate(Vector3d Ps[], Vector3d tic[], Matrix3d ric[])
{
    ...
    for (auto &it_per_id : feature)
    {
        ...
        Eigen::Matrix3d R0 = Rs[imu_i] * ric[0]; // Rwi * Ric = Rwc
        ...
        {
            imu_j++;
            // 将imu位姿变换到该KF的相机坐标系位姿
            // tic[0]和ric[0]则是相机的位置和旋转偏置，表示从相机坐标系相对于相机传感器坐标系的转换。Ps[imu_j] + Rs[imu_j] * tic[0] 表示将相机传感器坐标系的位置转换到相机坐标系下

            // 使用imu预积分的位姿作为初始位姿，并将初始位姿变换到相机坐标系
            Eigen::Vector3d t1 = Ps[imu_j] + Rs[imu_j] * tic[0];
            Eigen::Matrix3d R1 = Rs[imu_j] * ric[0];

            // 1. T_w_cj = T_c0_cj*T_w_c0 => T_c0_cj = inv(T_w_c0)*T_w_cj = trans(T_w_c0)*T_w_cj
            // 2. 正交矩阵： trans(A)*A= I (使用初等行变换进行单位化) 并且 inv(A)*A= I => trans(A) = inv(A)
            // 求位姿变换时是算 先旋转后平移，所以求逆变换时 先平移后旋转
            // T_w_cj -> T_c0_cj

            // 将所有看到特征点的关键帧位姿变换到第一个看到特征点的关键帧坐标系下
            Eigen::Vector3d t = R0.transpose() * (t1 - t0);
            Eigen::Matrix3d R = R0.transpose() * R1; // R_c0_cn
            Eigen::Matrix<double, 3, 4> P;
            // T_c0_cj -> T_cj_c0相当于把c0当作世界系
            // T_c0_cj 表示将坐标从c0变换到cj需要左乘T_c0_cj
            // 在上一帧坐标系下，计算位姿增量的逆（变换回去的位姿）
            // 计算逆变换
            P.leftCols<3>() = R.transpose();
            P.rightCols<1>() = -R.transpose() * t; // R_cn_c0
            Eigen::Vector3d f = it_per_frame.point.normalized();
            // 构建超定方程的其中两个方程
            // f: 像素坐标，p：世界坐标到c0参考系的变换
            // "p = inv（T_w_cj - T_w_c0）"

            /**
             *  构造系数矩阵A？
             * 对于三维坐标点 P 和 像素坐标点 u,相机光心为O,由于像素坐标点到光心的向量与三维坐标点到光心的向量是平行的（重合），所以 OP 与 Ou 的差积为 0 （差积的方向与垂直于原向量，其模长为原向量张成的平行四边形面积，平行向量张成的面积为0）
             * svd_A 为光心 oO 与 OP 叉乘的结果
             */
            // 在第一个关键帧坐标系下，构建系数矩阵
        ...
        Eigen::Vector4d svd_V = Eigen::JacobiSVD<Eigen::MatrixXd>(svd_A, Eigen::ComputeThinV).matrixV().rightCols<1>(); // 获取最后一列（特征值最小的一列）
        // 求解齐次坐标下的深度，Z/s
