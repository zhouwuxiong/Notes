...
public:
     ...
     *
     函数参数解释如下:
        double _dt: 两个时刻之间的时间间隔
        const Eigen::Vector3d &_acc_0: 初始时刻的线性加速度
        const Eigen::Vector3d &_gyr_0: 初始时刻的角速度
        const Eigen::Vector3d &_acc_1: 结束时刻的线性加速度
        const Eigen::Vector3d &_gyr_1: 结束时刻的角速度
        const Eigen::Vector3d &delta_p: 初始时刻的位置
        const Eigen::Quaterniond &delta_q：初始时刻的姿态
        const Eigen::Vector3d &delta_v：初始时刻的速度
        const Eigen::Vector3d &linearized_ba：初始时刻的加速度零偏
        const Eigen::Vector3d &linearized_bg：初始时刻的角速度零偏
        Eigen::Vector3d &result_delta_p：返回获取的IMU位置增量
        Eigen::Quaterniond &result_delta_q：返回获取的IMU姿态增量
        Eigen::Vector3d &result_delta_v：返回获取的IMU速度增量
        Eigen::Vector3d &result_linearized_ba：返回获取的加速度偏差增量
        Eigen::Vector3d &result_linearized_bg：返回获取的角速度偏差增量
        bool update_jacobian： 标志位, 是否计算雅克比矩阵
    ...
    变量说明：
        ...
        delta_q: 初始时刻的姿态（四元数）
        delta_v: 初始时刻的速度
        linearized_ba: 初始时刻的加速度偏移
        linearized_bg: 初始时刻的陀螺仪偏移
        un_acc_0: 初始时刻IMU加速度的运动学（测量）值
        un_gyr: IMU陀螺仪测量的角速度值
        un_acc_1：结束时刻IMU加速度的运动学（测量）值
        result_delta_p：结束时刻的位置
        result_delta_q：结束时刻的姿态（四元数）
        result_delta_v：结束时刻的速度
        result_linearized_ba：结束时刻的加速度偏移
        result_linearized_bg：结束时刻的陀螺偏移

    代码实现的过程是：
        1. 把IMU加速度测量值去掉加速度量级偏移值，与旋转矩阵delta_q相乘，得到加速度运动学（测量）值un_acc_0。

        2. 通过中值积分计算两个时刻内的旋转变化量result_delta_q。

        3. 通过delta和result_delta_q计算速度值的变化量，并加上时间的变化量_dt，得到结束时刻的速度值result_delta_v。

        4. 通过delta和result_delta_v和un_acc_0计算位置值的变化量，并加上时间的变化量_dt，并除以2，得到结束时刻的位置值result_delta_p。

        5. 最后，根据IMU的运动学（测量）值、陀螺仪偏移量和加速度偏移量，来更新方差矩阵及雅克比矩阵，用于后续的状态更新。

        欧拉积分： 用起始点的斜率 作为 t->t+delta_t 区间内的斜率
        中值积分： 用中点的斜率  作为 t->t+delta_t 区间内的斜率
        RK4积分(4阶龙格库塔法)： y(n+1) = y(n) + delta_y/6(K1+2K2+2K3+k4).其中
    ...
    */
                             ...
                             Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        //  首先中值积分更新状态量
        // 计算起点的imu加速度，并变换到imu坐标系下 （ un -> uniform : 均匀的）
        Vector3d un_acc_0 = delta_q * (_acc_0 - linearized_ba);
        //  计算 imu的平均角速度 （ 所以这里假设了角速度的变化是均匀的）
        Vector3d un_gyr = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;
        // Quaterniond类是Eigen库中表示四元数（Quaternion）的类，四元数由实部和虚部构成，在该计算中，传入的参数是一个由角速度计算得到的旋转四元数（在实部填充1.0表示无旋转，而在虚部则填充3个分量值），中间乘上时间间隔_dt/2即得到两个时刻之间的旋转差量增量。
        result_delta_q = delta_q * Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2);

        // 计算终点的加速度
        Vector3d un_acc_1 = result_delta_q * (_acc_1 - linearized_ba);
        // 计算平均加速度（中值），（ 所以这里假定了加速度的变化是均匀的 ？？）
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);
        // 使用平均加速度计算位移 x= x0+ v0*t + 1/2*a*t^2
        ...
        if (update_jacobian)
        {
            Vector3d w_x = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg; // 平均角速度
            Vector3d a_0_x = _acc_0 - linearized_ba;                // 起点加速度
            Vector3d a_1_x = _acc_1 - linearized_ba;                // 终点加速度
            ...
            // F矩阵：计算状态向量的微分变化，表示当前变化量对应的系统状态的增量。F矩阵系数对应IMU在两个时刻之间状态不同时各个状态变量的关系。
            ...
            // V矩阵：输入矢量的微分变化，表示当前输入量对应的系统状态的增量。V矩阵系数对应IMU状态中各个输入项增量时各个状态变量的关系。
            ...
            // step_V = V;
            jacobian = F * jacobian;
            // 噪声矩阵noise：表示由于外部环境干扰等因素而引入的噪声项，该矩阵是一个对称正定矩阵，用于描述噪声的方差。
